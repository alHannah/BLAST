...
if(isset($_POST['form1'])) {
    ...
    if(empty($_POST['region_id'])) {
		...
		foreach($result as $row) {
			$current_region = $row['region_id'];
		}

		$statement = $pdo->prepare("SELECT * FROM tbl_shipping_cost WHERE region_id=? and region_id!=?");
    	$statement->execute(array($_POST['region_id'],$current_region));
    ...
    if($valid == 1) {    	
		...
		$statement = $pdo->prepare("UPDATE tbl_shipping_cost SET region_id=?,amount=? WHERE shipping_cost_id=?");
		$statement->execute(array($_POST['region_id'],$_POST['amount'],$_REQUEST['id']));
                            ...
                            <div class="col-sm-4">
                                ...
                                foreach ($result as $row) {
                                    ...
                                    <option value="<?php echo $row['region_id']; ?>"
                                        <?php if($row['region_id'] == $region_id) {echo 'selected';} ?>>
