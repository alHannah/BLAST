		...
		<script src="https://ckeditor.com/ckeditor-5"></script>
		<script>
...
// Update mid categories based on top category selection
$(".top-cat").on('change', function() {
    const tcat_id = $(this).val();
    if (tcat_id) {
        $.ajax({
            type: "GET",
            ...
            success: function(html) {
                $(".mid-cat").html(html);
                $(".end-cat").html(
                    '<option value="">Select End Level Category</option>'); // Reset end categories
            },
            error: function() {
                alert("Failed to load mid categories.");
            }
        });
    } else {
...
// Update end categories based on mid category selection
$(".mid-cat").on('change', function() {
    const mcat_id = $(this).val();
    if (mcat_id) {
        $.ajax({
            type: "GET",
            url: "get-category.php",
            ...
            error: function() {
                alert("Failed to load end categories.");
            }
        });
    } else {
        $(".end-cat").html('<option value="">Select End Level Category</option>');
