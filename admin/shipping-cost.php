...
if(isset($_POST['form1'])) {
    ...
    if(empty($_POST['region_id'])) {
        $valid = 0;
        $error_message .= 'You must have to select a region.<br>';
    }
    ...
    if($valid == 1) {
        $statement = $pdo->prepare("INSERT INTO tbl_shipping_cost (region_id,amount) VALUES (?,?)");
        $statement->execute(array($_POST['region_id'],$_POST['amount']));
                            ...
                            <div class="col-sm-4">
                                <select name="region_id" class="form-control select2">
                                    ...
                                    foreach ($result as $row) {
                                        ...
                                        $statement = $pdo->prepare("SELECT * FROM tbl_shipping_cost WHERE region_id=?");
                                        $statement->execute(array($row['region_id']));
                                        ...
                                        ?>
                                    <option value="<?php echo $row['region_id']; ?>"><?php echo $row['region_name']; ?>
                                    </option>
                                    <?php
                                    }
                                    ?>
...
<section class="content">
    ...
    <div class="row">
        <div class="col-md-12">
            ...
            <h4 style="background: #dd4b39;color:#fff;padding:10px 20px;">NB: If a region does not exist in the above
